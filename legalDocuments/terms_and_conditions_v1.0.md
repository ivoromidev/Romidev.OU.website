# Terms of Service

Version: v1.1  
Effective Date: 2026-07-17  
Company: RomiDev OÜ (Estonia)  
Contact: info@romidev.ee  

---

## 1. Introduction

These Terms of Service (“Terms”) govern your use of the application provided by RomiDev OÜ (“we”, “us”, or “our”).

By using the application, you agree to these Terms. If you do not agree, you should not use the application.

---

## 2. Description of Service

The application provides productivity and calendar-related features, including:
- Calendar integration via Google Calendar API
- Contact name enrichment for calendar attendees via Google People API (Contacts)
- Note-taking and handwritten input
- AI-assisted or automated content features (if applicable)
- Analytics-based product improvements

The service is provided for personal and professional productivity use.

---

## 3. User Accounts

To use certain features, you may sign in using Google Sign-In.

You are responsible for:
- Maintaining the security of your account
- Ensuring the accuracy of information provided
- All activity performed through your account

We are not responsible for unauthorized access caused by user negligence.

---

## 4. User Content

You retain ownership of any content you create, including:
- Notes
- Handwritten input
- Calendar-related inputs or annotations

You are responsible for ensuring that your content does not violate applicable laws or third-party rights.

---

## 5. Data Storage

- User-created notes and handwritten content are stored locally on your device.
- We do not store user notes or personal content on our servers.
- Some metadata may be processed by third-party analytics providers.

---

## 6. Acceptable Use

You agree not to:
- Use the application for illegal purposes
- Attempt to reverse engineer or disrupt the service
- Interfere with security or integrity of the application
- Use the service in a way that harms other users or systems

---

## 7. Third-Party Services

The application integrates with third-party services, including:
- Google (Google Sign-In, Google Calendar API, Google People API / Contacts)
- Amplitude (analytics and usage tracking)

Use of these services is subject to their respective terms and privacy policies.

---

## 8. Subscription and Payments (If Applicable)

Some features may be offered on a paid basis in the future.

If subscriptions are introduced:
- Pricing and billing terms will be clearly displayed in the application or App Store listing
- Payments will be processed through Apple’s App Store payment system

---

## 9. Availability

We aim to provide reliable access to the application but do not guarantee:
- Continuous availability
- Error-free operation
- Compatibility with all devices or configurations

We may modify or discontinue features at any time.

---

## 10. Disclaimer of Warranties

The application is provided “as is” and “as available” without warranties of any kind, express or implied.

We do not guarantee:
- Accuracy of results
- Fitness for a particular purpose
- Uninterrupted or error-free operation

---

## 11. Limitation of Liability

To the maximum extent permitted by law, RomiDev OÜ shall not be liable for:
- Indirect or incidental damages
- Loss of data
- Loss of profits or business opportunities
- Damages resulting from use or inability to use the application

---

## 12. Termination

We may suspend or terminate access to the application if:
- You violate these Terms
- Required by law or regulatory authority
- Necessary to protect the integrity of the service

You may stop using the application at any time.

---

## 13. Changes to Terms

We may update these Terms from time to time. Updated versions will be published within the application or on the official website with a revised effective date.

Continued use of the application constitutes acceptance of the updated Terms.

---

## 14. Governing Law

These Terms are governed by the laws of Estonia, without regard to conflict of law principles.

---

## 15. Contact

RomiDev OÜ  
info@romidev.ee